...
CstaToolkit.prototype.serializeCreateWebSession = function(roomId, name,callGroup,token, browser){
	...
	return `<?xml version="1.0" encoding="utf-8"?><loginRequest type="User" callGroupName="${callGroup}" platform="WebChat" webToken="${token}" forced='true' version="1.0" abNotify="true" dispName="${name}" clientType="Web"
	  loginCapab="Im|WebChat|ScreenSharing|HttpFileTransfer"  apiVersion="17"><userName>webuser_${name}</userName><logging><browser>${browser}</browser></logging></loginRequest>`
}
